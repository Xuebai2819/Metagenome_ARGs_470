######################### Procrustes_analyses ###################################
data <- read.csv("card_tax_reads_number.csv",row.names = 1, header = T)
### Sum rows with the same Species and duplicate CARD_ARO
 #Remove the last column sum value
data_df <- data[,c(-1:-7,-10:-15,-486)]
result <- data_df %>%
  group_by(Species, CARD_ARO) %>%
  summarise_all(sum)
#
### Deduplication
table_name <- data[,1:15]
table_name_result <- table_name %>%
  group_by(Species) %>%
  distinct(CARD_ARO, .keep_all = TRUE)
#
# Merge the column name table and the summed table
merge_table <- merge(table_name_result, result, by = c("Species", "CARD_ARO"))
# write out file
write.csv(merge_table, "card_tax_reads_number_sum.csv")

#Read in the tax_species name table to be extracted
tax_species <- read.csv("tax_s_all_relative_abundance_0.97_filter_species.csv", row.names = 1, header = T)
# Extract species names
index <- rownames(tax_species)
# Build an empty data frame df
df <- data.frame()
# Loop to extract ARGs information corresponding to species
for (i in index) { #i = "s__Escherichia_coli"
  ff <- merge_table[merge_table$Species == i,]
  df <- rbind(df, ff)
}
# Same sum for CARD_ARO
df_1 <- df[,c(-1,-3:-15)]
filter_sum <- df_1 %>%
  group_by(CARD_ARO) %>%
  mutate_all(as.numeric) %>%
  summarise_all(sum)
write.csv(filter_sum, "species_filter_ARO_Procrustes_analyses.csv", row.names = FALSE)
